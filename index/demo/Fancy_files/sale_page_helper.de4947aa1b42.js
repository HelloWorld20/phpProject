jQuery(function($){
	...
	$('#content,#sidebar,#popup_container').delegate('.show_cart select[name=option_id]','change',function(event){
		...
		var $qtySelect = $this.closest('.show_cart').find("select[name=quantity]").empty();
	...
	$('#content,#sidebar,#popup_container').delegate('.show_cart button.btn-notify','click',function(event){
        ...
        var option_id = $this.closest(".show_cart").find('select[name=option_id]').val();
			...
			success  : function(json){
				...
				if (remove == 1) {
				    ...
				    if ("option_id" in params) {
				    	$this.closest(".show_cart").find("select[name=option_id] option:selected").attr("waiting","False");
				    }
				} else {
				    $this.addClass('subscribed').text(gettext("Subscribed"));
				    $this.closest(".show_cart").find("select[name=option_id] option:selected").attr("waiting","True");				    
				}
	...
	$('#content,#sidebar,#popup_container,#slideshow').delegate('#add_to_cart,.add_to_cart','click',function(event){
        ...
        try{ 
            ...
            mixpanel_log('Add to Cart', mixpanel_param);
        } catch(e) {};
        if (dataLayer) {
            dataLayer.push({'event': 'Add_to_Cart_Button', 'product_id': param['sale_item_id'], 'products': undefined, 'products_info': undefined, 'revenue': undefined });
        }
	...
	$(document).delegate('.same-delivery .same-day-shipping-pop','click',function(){
        ...
        $.dialog('delivery-popup').open().$obj        	
                .on('click', '.ly-close', function(e) {
                    close_delivery();
                    e.preventDefault();
                    e.stopPropagation();
                });
	...
	$(document).delegate('.delivery-popup .check-same-day-shipping','click',function(){
        var sale_item_id = $(this).parents('.delivery-popup').attr('si-id');
        var country_code = $(this).parents('.delivery-popup').find('select.same-day-country-select option:selected').val();
		...
		if (zip.length>0){
		    ...
		    $.post("/check-same-day-eligibility.json", param, 
		    function(response){
			    if (response.status_code != undefined && response.status_code == 1) {
                    $('.delivery-popup fieldset').before('<p class="notify-delivery success"><i class="check"></i> <b>Good news!</b> Your address is eligible for Same-Day Delivery.</p>')
			    }
                else if (response.status_code != undefined && response.status_code == 2) { 
                    $('.delivery-popup fieldset').before('<p class="notify-delivery"><i class="not"></i> '+"<b>Hang tight, we're almost there!</b> Unfortunately same-day delivery is not currently available in your area, but we're expanding rapidly.</p>")
                }
                else if (response.status_code != undefined && response.status_code == 3) {
                    $('.delivery-popup fieldset').before('<p class="notify-delivery"><i class="not"></i> '+"We support overnight delivery!</p>")
                }
                else if (response.status_code != undefined && response.status_code == 4) {
                    $('.delivery-popup fieldset').before('<p class="notify-delivery"><i class="not"></i> '+"We support expedited shipping!</p>")
                }
	...
	$(document)
		.on('click', 'span.show_cart .btn-cart', function(event){
			event.preventDefault();
		...
		.on('click', 'span.show_cart .trick', function(event){
			event.preventDefault();
			$(this).closest('li').removeClass('active').find('.opened').removeClass('opened').find('.sale-item-input').removeClass('bot');
		})
        .on('mixpanel', 'span.show_cart', function(event) {
            var $this = $(this), $add_to_cart = $this.closest('.show_cart').find('.add_to_cart');
            var param = { 'thing id': $add_to_cart.attr('tid'), 'sale id': $add_to_cart.attr('sii'), 'via': $add_to_cart.attr('via')};
            ...
            try { mixpanel_log('Buy', param); } catch(e) { console.log(e) }
        });
